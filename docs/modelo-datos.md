...
| `rol`           | VARCHAR(20)   | NOT NULL          | Valores: `admin`, `operador`.                          |
...
- `operador` tiene una o más entradas → solo ve esas sedes.
...
| `rol` como VARCHAR en `usuarios` | Dos roles fijos (`admin`, `operador`). Suficiente para académico; en producción se normalizaría a tabla `roles`. |
